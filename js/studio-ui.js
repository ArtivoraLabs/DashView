...
(function () {
  ...
  function computeDataHealth() {
    ...
    if (emptyCols.length) issues.push({ type: 'empty-cols', text: emptyCols.length + ' column' + (emptyCols.length > 1 ? 's are' : ' is') + ' completely empty: ' + emptyCols.join(', '), action: 'Remove them' });
    ds.fields.forEach(function (f) {
      var pct = f.nulls / ds.rowCount;
      if (pct > 0.3 && pct < 1) issues.push({ type: 'nulls', text: '"' + f.name + '" is missing in ' + Math.round(pct * 100) + '% of rows' });
    });
    var sig = {}, dupCount = 0;
    ds.typedRows.forEach(function (row) {
      var key = ds.fields.filter(function (f) { return !f.isCalculated && !f.isVirtual; }).map(function (f) { return row[f.name]; }).join('|');
      sig[key] = (sig[key] || 0) + 1;
    });
    Object.keys(sig).forEach(function (k) { if (sig[k] > 1) dupCount += sig[k] - 1; });
    if (dupCount > 0) issues.push({ type: 'duplicates', text: dupCount + ' likely duplicate row' + (dupCount > 1 ? 's' : '') + ' found', action: 'Remove duplicates' });
  ...
  function renderDataHealthModal() {
    ...
    if (!issues.length) { body.innerHTML = '<div class="health-all-good">' + ICON.check.replace('width="24" height="24"', '') + '<p>No issues found — your data looks clean.</p></div>'; return; }
    body.innerHTML = issues.map(function (iss, i) {
      return '<div class="health-item">' + ICON.alert + '<div class="health-item-body"><p>' + esc(iss.text) + '</p></div>'
  ...
  on(byId('dataHealthBody'), 'click', function (e) {
    ...
    if (iss.type === 'empty-cols') {
      ...
      ds.typedRows.forEach(function (row) {
        var key = ds.fields.filter(function (f) { return !f.isCalculated && !f.isVirtual; }).map(function (f) { return row[f.name]; }).join('|');
        if (seen.has(key)) return; seen.add(key); kept.push(row);
      });
      ds.typedRows = kept; ds.rowCount = kept.length;
      Studio.recomputeAllStats(ds);
      markDirty(); renderAll(); renderDataHealthModal(); showToast('Duplicate rows removed.', 'success');
    }
  });
  ...
  function ingest(columns, rawRows, fileName) {
    ...
    var issues = computeDataHealth();
    if (issues.length) showToast('Imported ' + ds.rowCount.toLocaleString() + ' rows. Found ' + issues.length + ' thing' + (issues.length > 1 ? 's' : '') + ' worth a look — see Data health.', 'info');
    else showToast('Imported ' + ds.rowCount.toLocaleString() + ' rows and ' + ds.fields.length + ' columns.', 'success');
